...
async function sendNotesToNotion(data) {
  ...
  const databaseId = "b87fd219bae84edcbe4b73469518788b";
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to create Notion entry: ${response.status} - ${response.statusText}`,
      );
    }
...
async function executeTasks() {
  await clickSubmitButton()
    ...
    .then((paraphrased) => sendNotesToNotion(paraphrased))
    .then((resp) => {
      console.log("Sent notes to Notion", resp);
    })
    .catch((error) => {
